...
function testMerge(startDoc, endDoc1, endDoc2, automergeSteps, conflictingSteps1, conflictingSteps2, options: Options = {}) {
    ...
    assert.equal(
        JSON.stringify(merge.tr.steps.map(step => step.toJSON())),
        JSON.stringify(automergeSteps)
    );

    assert.equal(
        JSON.stringify(merge.merge.conflictingSteps1.map(step => (step[1] ? step[1].toJSON() : false)).filter(step => step)),
        JSON.stringify(conflictingSteps1)
    );

    assert.equal(
        JSON.stringify(merge.merge.conflictingSteps2.map(step => (step[1] ? step[1].toJSON() : false)).filter(step => step)),
        JSON.stringify(conflictingSteps2)
    );
}
